...
class backup_groupexchange_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $offergroups = new backup_nested_element('offer_groups');

        $offergroup = new backup_nested_element('offer_group', array('id'), array('groupid'));
        ...
        $offer->add_child($offergroups);
        $offergroups->add_child($offergroup);
        ...
        if ($userinfo) {
            ...
            $offergroup->set_source_table('groupexchange_offers_groups', array('offerid' => '../../id'));
        }

        // Define id annotations.
        $offer->annotate_ids('user', 'userid');
        $offer->annotate_ids('group', 'group_offered');
        $offergroup->annotate_ids('group', 'groupid');
